...
# 经过上一版本，有一个初步的程序了，但是误检较为严重，所以，现在尝试去除滤波进行测试
...
# 先对图片做一些简单的预处理
# 读出预先准备的图片
img = cv2.imread(r'D:\pythonProject\pythonLearning\gameCheat\findDifference\picSource\pic4.PNG', cv2.IMREAD_COLOR)
# 转换图片的格式，从BGR转换为HSV
...
_, _, last_stats, last_centroids = cv2.connectedComponentsWithStats(dst_closing)
